...
def get_red_pajama(nsamples, seqlen, tokenizer, eval_mode=False, subsample=-1):
    ...
    if subsample == -1:
        ...
        for count in trange(nsamples*8, desc="Making red_pajama calibration set", leave=False):
            while True:
                i = random.randint(0, len(traindata) - 1)
                trainenc = tokenizer(traindata[i]["text"], return_tensors="pt")
                if trainenc.input_ids.shape[1] > seqlen:
            ...
            i = random.randint(0, trainenc.input_ids.shape[1] - seqlen - 1)
            j = i + seqlen
            if subsample*nsamples <= count and count < (subsample+1)*nsamples:
                inp = trainenc.input_ids[:, i:j]
                assert inp.shape[1] == seqlen
                trainloader.append(inp)
    return trainloader
...
def get_loaders(name, nsamples=128, seed=0, seqlen=2048, eval_mode=False, model_path=None, subsample=-1):
    """
    ...
    #     data = []
    #     with gzip.open("/home/lj9979/PyTorch-Pretrained-ViT/examples/sentiment_data/AMAZON_FASHION_5.json.gz", 'rb') as gzip_file:
    ...
    if name.lower() == "none":
        ...
        orig_data = torch_datasets.ImageFolder("/home/lj9979/PyTorch-Pretrained-ViT/examples/imagenet/data/val", transform = val_transforms)
        data = torch.utils.data.Subset(orig_data, range(100))
    else:
        ...
        if name.lower() == "wikitext2":
            ...
            data = get_red_pajama(nsamples, seqlen, tokenizer, eval_mode=eval_mode, subsample=subsample)
        elif name.lower() == "ptb":
